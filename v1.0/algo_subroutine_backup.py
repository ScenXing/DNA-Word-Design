...
def binarySearchLength14(n, k1, k4, initLength, pascalPrefixSum):
	k = max(k1, k4)
	def compute(l):
		result = fractions.Fraction(pascalPrefixSum[l][k - 1], helper.fastPower(2, l))
		
		powTwo = helper.fastPower(2, l - k4 + 1)
		sumTemp = fractions.Fraction(0, 1)
		for i in xrange(l - k4 + 1, l):
			sumTemp += fractions.Fraction(pascalPrefixSum[i][k4 - (l - i) - 1], powTwo)
			powTwo *= 2
 		sumTemp *= 2
...
def computeExpCountEmpty(n, l, k1, k4, pascalPrefixSum):
	k = max(k1, k4)	
	firstTerm = fractions.Fraction(((n * (n - 1)) / 2) * (1 + 2 * (k4 - 1)), 1)

	secondTerm = fractions.Fraction(pascalPrefixSum[l][k - 1], helper.fastPower(2, l))
...
def computeChangeInExpCount(M, strPos, strId, newVal, unknownChar, numDiff1, numDiff4, n, l, k1, k4, pascalPrefixSum):
	...
	totalDiff = fractions.Fraction(0, 1);
	...
	for curStrId in xrange(strId):
		...
		totalDiff += computeChangeInProb(l, k, strPos, oldNumDiff, newVal != M[curStrId][strPos], pascalPrefixSum, debug)
...
################################################################################
#
def computeChangeInProb(l, k, s, t, diffIncrease, pascalPrefixSum, debug=False):
	if t > k - 1:
